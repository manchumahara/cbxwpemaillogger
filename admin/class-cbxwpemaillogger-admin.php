...
class CBXWPEmailLogger_Admin {
			...
			'cbxwpemaillogger_email' => array(
				...
				'mailer'                => array(
					...
					'options' => array(
						...
						'smtp'    => esc_html__( 'Custom SMTP', 'cbxwpemaillogger' ),
					),
				),
	...
	public function phpmailer_init_extend( $phpmailer ) {
		...
		if ( $email_smtp_enable ) {
			...
			if ( $mailer == 'smtp' ) {
				...
				$host   = sanitize_text_field( $setting->get_option( 'smtp_email_host', 'cbxwpemaillogger_smtps', 'localhost' ) );
				$port   = intval( sanitize_text_field( $setting->get_option( 'smtp_email_port', 'cbxwpemaillogger_smtps', '25' ) ) );
				$secure = esc_attr( sanitize_text_field( $setting->get_option( 'smtp_email_secure', 'cbxwpemaillogger_smtps', 'none' ) ) );
				if ( $secure == 'none' ) {
					$secure = '';
				}

				$auth = intval( sanitize_text_field( $setting->get_option( 'smtp_email_auth', 'cbxwpemaillogger_smtps', 0 ) ) );


				$username = esc_attr( sanitize_text_field( $setting->get_option( 'smtp_email_username', 'cbxwpemaillogger_smtps', '' ) ) );
				$password = esc_attr( sanitize_text_field( $setting->get_option( 'smtp_email_password', 'cbxwpemaillogger_smtps', '' ) ) );
	...
	public function plugin_activate_upgrade_notices() {
		...
		if ( get_transient( 'cbxwpemaillogger_activated_notice' ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Thanks for installing/deactivating CBX Email SMTP & Logger V%s - <a href="%s" target="_blank">Codeboxr Team</a>', 'cbxwpemaillogger' ), CBXWPEMAILLOGGER_PLUGIN_VERSION, 'https://codeboxr.com' ) . '</p></div>';
		...
		if ( get_transient( 'cbxwpemaillogger_upgraded_notice' ) ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . sprintf(__( 'Thanks for upgrading CBX Email SMTP & Logger V%s , enjoy the new features and bug fixes - <a href="%s" target="_blank">Codeboxr Team</a>', 'cbxwpemaillogger' ), CBXWPEMAILLOGGER_PLUGIN_VERSION, 'https://codeboxr.com' ) . '</p></div>';
			// Delete the transient so we don't keep displaying the activation message
